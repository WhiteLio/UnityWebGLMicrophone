...
public class PitchDsp
{
    ...
    public const int kMinMidiNote = 19;  // A0
    public const int kMaxMidiNote = 106; // C8
    private const int correctionOffset = 2;
    ...
    private float DetectPitchLo(float[] samplesLo, float[] samplesHi)
    {
        ...
        for (int idx = 0; idx < m_numCourseSteps; idx += skipSize)
        {
            ...
            if (loBuffer)
            {
                if (!bufferSwitched)
                {
                    Array.Clear(m_detectCurve, 258 - peakScanSizeHalf, 258 + peakScanSizeHalf);
                    bufferSwitched = true;
                }
